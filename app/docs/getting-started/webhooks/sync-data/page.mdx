...
<InfoCard
  ...
  items={[
    { icon: User, label: "Trackify account is required", href: "http://localhost:3000/register" },
    { icon: Globe, label: "Ngrok account is required", href: "https://dashboard.ngrok.com/signup" },
    { icon: Lock, label: "A backend server (Node.js, Python, etc.) to receive webhooks" }
  ]}
/>
...
Webhooks allow Trackify to notify your application in real time when important events happen, such as when a link is clicked, created, updated, or deleted. This guide will walk you through setting up a webhook endpoint, testing it locally with ngrok, securing it with a signing secret, and preparing for production.
...
Create an endpoint in your backend to receive POST requests from Trackify. Here’s a simple Node.js/Express example:
...
- [See all supported webhook events](/docs/webhooks/events)
- [Learn more about securing your webhooks](/docs/webhooks/security)
